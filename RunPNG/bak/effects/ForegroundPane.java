...
{
	public ForegroundPane(EventHandler<EffectAction> handlerEffectAction)
	{
		...
		label = createRowLabel("label.foreground.pic");
		add(label, 0, 3, 1, 1);

		m_buttonPicLoad = createImageButton("foreground.pic.open", "tooltip.foreground.load", "icons/32x32/file.open.png", action -> { handlePicLoad(); });
		add(m_buttonPicLoad, 1, 3, 1, 1);

		m_buttonPicRemove = createImageButton("foreground.pic.remove", "tooltip.foreground.remove", "icons/32x32/trashbin.png", action -> { handlePicRemove(); });
		...
		m_buttonScaleSym = createImageButton("foreground.pic.scale.both", "tooltip.scale.sym", "icons/32x32/scale.symmetrical.png", action -> { handleScaleSymmetrical(); });
		m_vboxImageScale.getChildren().add(m_buttonScaleSym);

		m_buttonScaleBoth = createImageButton("foreground.pic.scale.both", "tooltip.scale.both", "icons/32x32/scale.both.png", action -> { handleScaleBoth(); });
		m_vboxImageScale.getChildren().add(m_buttonScaleBoth);

		m_buttonScaleHorz = createImageButton("foreground.pic.scale.horz", "tooltip.scale.horz", "icons/32x32/scale.horz.png", action -> { handleScaleHorz(); });
		m_vboxImageScale.getChildren().add(m_buttonScaleHorz);

		m_buttonScaleVert = createImageButton("foreground.pic.scale.vert", "tooltip.scale.vert", "icons/32x32/scale.vert.png", action -> { handleScaleVert(); });
		m_vboxImageScale.getChildren().add(m_buttonScaleVert);

		m_buttonRevert = createImageButton("foreground.pic.scale.vert", "tooltip.revert", "icons/32x32/revert.png", action -> { handleRevert(); });
		...
		m_thumbnail.setBaseImage(m_image);
	...
	public Image applyEffect(int nFrame, Image image)
	{
		...
		return ImageUtil.imageApplyForeground(image, m_pickerPlain.getValue(), m_thumbnail.getViewImage());
	}
}
